...
import java.util.logging.Level; // Added import statement for Level
...
public class BookMark {
    ...
    // @@author hailey-jung
    /**
     * Marks a book as read by its index and records the completion date.
     *
     ...
     */
    public static void markDoneByIndex(BookList bookList, int index, LocalDate completionDate)
            throws IndexOutOfBoundsException, BookReadAlreadyException {
        try {
            ...
            markBookAsRead(bookList.getBooks().get(index - 1), completionDate);
    ...
    /**
     * Marks the book as read and records the completion date.
     *
     * @param book The book to mark as read.
     * @param completionDate The completion date of reading the book.
     */
    public static void markBookAsRead(BookMain book, LocalDate completionDate) {
        // Assuming setCompletionDate method is available in BookMain
        // If not, you need to implement it in BookMain
        book.setCompletionDate(completionDate); // Set completion date
        Read.setRead(book, true);
        System.out.println("Successfully marked " + Title.getTitle(book) + " as read on " + completionDate + ".");
    }
